class Api::BaseController < ActionController::API
  # Enable cookies for same-origin API requests
  ...
  def find_session_record
    # Try cookie-based authentication first (for same-origin requests)
    if cookies.signed[:session_token].present?
      return Session.find_by_id(cookies.signed[:session_token])
    end

    # Try Authorization header authentication (for API clients)
